...
class KitchenDesign extends Component {
  ...
  getDesignImages = async () => {
    try {
      ...
      let kitchenFilter = data.filter(
        (filter) => filter.imageType.indexOf("Kitchen") !== -1
      );

      const slice = kitchenFilter.slice(this.state.offset, this.state.offset + this.state.perPage)
      ...
      if (response.data.length !== 0) {
        this.setState({
          ...
          isEmpty: false,
          pageCount: Math.ceil(kitchenFilter.length / this.state.perPage),
          postData
        });
      } else {
        this.setState({
