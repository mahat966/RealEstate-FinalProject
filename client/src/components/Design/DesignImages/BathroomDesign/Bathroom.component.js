...
class BathroomDesign extends Component {
  constructor(props) {
    super(props);
    this.state = {
      designImages: [],
  ...
  getDesignImages = async () => {
    try {
      ...
      let bathroomFilter = data.filter(
        (filter) => filter.imageType.indexOf("Bathroom") !== -1
      );

      const slice = bathroomFilter.slice(this.state.offset, this.state.offset + this.state.perPage)
      ...
      if (response.data.length !== 0) {
        this.setState({
          ...
          isEmpty: false,
          pageCount: Math.ceil(bathroomFilter.length / this.state.perPage),
          postData
        });
      } else {
        this.setState({
