...
class HomeDesign extends Component {
  ...
  getDesignImages = async () => {
    try {
      ...
      let homeDesignFilter = data.filter(
        (filter) => filter.imageType.indexOf("Home Design") !== -1
      );

      const slice = homeDesignFilter.slice(this.state.offset, this.state.offset + this.state.perPage)
      ...
      if (response.data.length !== 0) {
        this.setState({
          ...
          isEmpty: false,
          pageCount: Math.ceil(homeDesignFilter.length / this.state.perPage),
          postData
        });
      } else {
        this.setState({
