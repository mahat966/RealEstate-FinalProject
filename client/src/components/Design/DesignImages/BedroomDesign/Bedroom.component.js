...
class BedroomDesign extends Component {
  ...
  getDesignImages = async () => {
    try {
      ...
      let bedroomFilter = data.filter(
        (filter) => filter.imageType.indexOf("Bedroom") !== -1
      );

      const slice = bedroomFilter.slice(this.state.offset, this.state.offset + this.state.perPage)
      ...
      if (response.data.length !== 0) {
        this.setState({
          ...
          isEmpty: false,
          pageCount: Math.ceil(bedroomFilter.length / this.state.perPage),
          postData
        });
      } else {
        this.setState({
